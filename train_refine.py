...
def calculate_gen_loss(fine_pred, Mask, Maskmimi, label):
    # Recon loss
    recon_loss_L1 = gen_loss(fine_pred*(Maskmimi), label*(Maskmimi)) 
    recon_loss_DC = gen_loss(fine_pred*(Mask), label*(Mask)) 
    ...
    # Total loss
    finalg_loss = recon_loss_L1 + recon_loss_DC + ( 0.01* fakeg_loss) + (0.1*lapla_loss)
